...
export const editAsyncNotes = (data: any) => async (dispatch: Dispatch<NoteActions>, getState: () => IState) => {
  ...
  if (res.status === 200) {
    dispatch(editNote({ title: data.title, text: data.text, tags: data.tags, groupId: data.groupId, headerImg: data.headerImg }))
    socketRef.emit('editNote', { title: data.title, text: data.text, tags: data.tags, groupId: data.groupId, headerImg: data.headerImg, selectNoteId })
  }
}
